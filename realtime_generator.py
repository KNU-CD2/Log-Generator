...
def output():
    file_path = "C:/Users/Boat/Desktop/ㅈㅂ/종프2/sample2.log"#파일경로설정
    ...
    while True:
        ...
        if mdate.hour < 8 or mdate.hour > 22: #5분 재움
            time.sleep(300)
            continue
        ...
        if mdate.hour == 8 or mdate.hour == 12:
            stime = random.randint(1, 2)
            # stime = random.randint(200, 250)
        else:
            stime = random.randint(1, 2)
            # stime = random.randint(250, 300)
        time.sleep(stime)
